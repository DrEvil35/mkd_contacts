# Контактная карта ответственных лиц

Версия v27.

## Исправление

В `index.html` убраны статичные значения из:

- `important-title`
- `important-summary`
- `page-title`
- `description`
- `footer`
- `top-line`

Теперь эти элементы полностью заполняются из `contacts.json`.

## Важно

Файл называется `contacts.json`, не `contact.js`.

Поле для подзаголовка блока «Важно»:

```json
"important": {
  "summary": "Актуальная информация по голосованию"
}
```

В `script.js` оно подставляется так:

```js
importantSummary.textContent = important.summary || '';
```

## Как обновить сайт

Замените файлы в корне репозитория GitHub Pages:

- `index.html`
- `style.css`
- `script.js`
- `contacts.json`
- `README.md`


## Версия v15

Добавлен более выразительный фон основной карточки:

- многослойный мягкий градиент;
- повторяющийся орнамент из точек и диагональных линий;
- лёгкая бумажная текстура;
- световые пятна без нарушения читаемости;
- сохранена существующая спокойная палитра v14.


## Версия v16

Изменения:

- из `script.js` удалены встроенные fallback-данные;
- сайт теперь берёт данные только из `contacts.json`;
- при ошибке загрузки `contacts.json` показывается понятное сообщение;
- `.description::after` расширен до `85%` ширины блока.


## Версия v17

Добавлен декоративный слой `flower-ornament` внутри основной карточки.

Особенности:
- используется тот же символ `✿`, что и в верхней строке;
- цветочки имеют разные размеры, прозрачность и поворот;
- расположение выглядит псевдослучайным, но остаётся статичным;
- слой декоративный и не мешает кликам, потому что `pointer-events: none`;
- контент остаётся выше орнамента за счёт `z-index`.


## Версия v21

Ссылки теперь управляются через справочник `contacts.json`.

Для каждого контакта можно указать:

```json
{
  "type": "vk",
  "url": "https://vk.com/id88635166",
  "appUrl": "vkontakte://profile/id88635166",
  "label": "Написать в VK"
}
```

Правило работы:

- `url` — обычная ссылка для браузера;
- `appUrl` — deeplink для Android;
- если Android и `appUrl` заполнен, используется `appUrl`;
- иначе используется обычный `url`.

Для Telegram:

```json
{
  "type": "telegram",
  "url": "https://t.me/Haskell35",
  "appUrl": "tg://resolve?domain=Haskell35",
  "label": "Написать в Telegram"
}
```

Для MAX `appUrl` не добавлен, потому что нужна подтверждённая схема приложения.


## Версия v23

Реализована логика:

- на компьютере ссылка всегда открывается как обычная HTTPS-ссылка из `url`;
- на мобильных устройствах iOS/Android при наличии `appUrl` сначала пробуется deeplink;
- если приложение не открылось, через короткую паузу происходит fallback на обычный `url`.

Структура контакта:

```json
{
  "type": "vk",
  "url": "https://m.vk.com/id88635166",
  "appUrl": "vkontakte://profile/id88635166",
  "label": "Написать в VK"
}
```

Для Telegram:

```json
{
  "type": "telegram",
  "url": "https://t.me/Haskell35",
  "appUrl": "tg://resolve?domain=Haskell35",
  "label": "Написать в Telegram"
}
```

Для MAX `appUrl` не добавляется, пока нет подтверждённой рабочей схемы приложения.


## Версия v24

Логика `url` / `appUrl` стала полностью универсальной.

Теперь сайт НЕ привязывает поведение к `type`.

Правило:

```text
desktop:
  всегда открывается link.url

mobile iOS / Android:
  если у конкретной записи контакта есть appUrl → пробуем appUrl
  если appUrl нет → открывается link.url
  если appUrl не открылся → fallback на link.url
```

`type` теперь используется только для визуального оформления кнопки:
`vk`, `telegram`, `max`, `phone`, `email`, `whatsapp` и т.д.

Пример универсальной записи:

```json
{
  "type": "custom",
  "url": "https://example.com/user",
  "appUrl": "exampleapp://user",
  "label": "Открыть контакт"
}
```

Пример записи только для браузера:

```json
{
  "type": "max",
  "url": "https://max.ru/u/example",
  "label": "Написать в MAX"
}
```

Вторая запись не содержит `appUrl`, поэтому на мобильном и на компьютере будет открываться обычный `url`.


## Версия v25

Справочник теперь только один:

- `contacts.json`

Файл `contacts.js` исключён из архива и больше не используется.

Сайт загружает данные только так:

```js
const response = await fetch('contacts.json', { cache: 'no-store' });
pageData = await response.json();
renderPage(pageData);
```

Для обновления данных нужно редактировать только `contacts.json`.


## Версия v26

Внедрён DeepLinker-подход с логированием в консоль.

Логика основана на событиях браузера:

- `blur` — браузер потерял фокус;
- `visibilitychange: hidden` — страница ушла в фон, приложение вероятно открылось;
- `focus` — пользователь вернулся или системный диалог закрылся.

В консоль пишутся сообщения с префиксом:

```text
[DeepLinker]
```

Поведение:

```text
desktop:
  обычный href = link.url

mobile iOS / Android:
  если у конкретного контакта есть appUrl → пробуем appUrl
  если appUrl нет → обычный href = link.url
  если appUrl не открылся → fallback на link.url
```

`type` не влияет на выбор ссылки. Он используется только для внешнего вида кнопки.

Справочник остаётся только в `contacts.json`.


## Версия v27

Исправление:

- имя справочника везде приведено к `contacts.json`;
- в архиве есть только JSON-справочник, без JS-дубликата;
- справочник проверен как валидный JSON.
